<dialog id="delete-department-modal" class="modal">
    <div class="bg-white rounded-xl shadow-xl max-w-sm w-full">
        <form method="POST" id="delete-department-form">
            ...
            <div class="p-6 text-center">
                ...
                <p class="text-sm font-medium text-slate-700" id="delete-department-target"></p>
                <p class="text-xs text-slate-400 mt-3">This action cannot be undone. Cannot delete if still referenced by programs, curricula or academic terms.</p>
            </div>
            <div class="flex justify-center gap-2 p-6 pt-2">
                <button type="button" onclick="document.getElementById('delete-department-modal').close()" class="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
