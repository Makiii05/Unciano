<dialog id="delete-offering-modal" class="modal">
    <div class="bg-white rounded-xl shadow-xl max-w-sm w-full">
        <form method="POST" id="delete-offering-form">
            <?= csrf_field() ?>
            <input type="hidden" name="subject_offering_id" id="delete-offering-id">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-slate-800 mb-2">Delete Offering</h3>
                <p class="text-sm text-slate-500 mb-1">Are you sure you want to delete</p>
                <p class="text-sm font-medium text-slate-700" id="delete-offering-target"></p>
                <p class="text-xs text-slate-400 mt-3">Cannot delete if used by enlistments or teacher offerings.</p>
            </div>
            <div class="flex justify-center gap-2 p-6 pt-2">
                <button type="button" onclick="document.getElementById('delete-offering-modal').close()" class="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">Cancel</button>
